...
class TPVAggregator(nn.Module):
    ...
    def forward(self, tpv_list, pointss=None):
        ...
        tpv_list[2]: bs, w*z, c
        """
        tpv_yz, tpv_xz, tpv_xy = tpv_list[0], tpv_list[1], tpv_list[2]
        bs, _, c = tpv_yz.shape #B,L,C
        tpv_yz = tpv_yz.permute(0, 2, 1).reshape(bs, c, self.tpv_h, self.tpv_w) #B,C,R,R
        tpv_xz = tpv_xz.permute(0, 2, 1).reshape(bs, c, self.tpv_z, self.tpv_h)
        tpv_xy = tpv_xy.permute(0, 2, 1).reshape(bs, c, self.tpv_w, self.tpv_z)
        ...
        if pointss is not None:
            # points: bs, n, 3
            _, n, _ = pointss.shape
            points = pointss.reshape(bs, 1, n, 3) #B,N,3 ~(0,R-1)
            points[..., 0] = points[..., 0] / (self.tpv_w) *2 -1
            points[..., 1] = points[..., 1] / (self.tpv_h) *2 -1
            points[..., 2] = points[..., 2] / (self.tpv_z) *2 -1
            sample_loc = points[:, :, :, [2, 1]] #y,z
            # grid_sample 是从（-1 ~ +1） ！！！！！！！
            tpv_yz_pts = F.grid_sample(tpv_yz, sample_loc).squeeze(2) # bs, c, n
            sample_loc = points[:, :, :, [2, 0]] #x,z
            tpv_xz_pts = F.grid_sample(tpv_xz, sample_loc).squeeze(2)
            sample_loc = points[:, :, :, [1, 0]] #x,y
            ...
            fused_pts = tpv_yz_pts + tpv_xz_pts + tpv_xy_pts
            #fused = torch.cat([fused_vox, fused_pts], dim=-1) # bs, c, whz+n
            fused = fused_pts.permute(0, 2, 1)

            return fused
        else:
            tpv_zh_vox = tpv_xz.unsqueeze(-1).permute(0, 1, 2, 4, 3).expand(-1, -1, -1, self.scale_h*self.tpv_h, -1)
