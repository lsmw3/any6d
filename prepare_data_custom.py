...
def process_category(category_path, output_path, feature_model_path, id_info):
    """
    ...
    for i, obj_folder in enumerate(tqdm(object_folders, desc=f"Processing {category_name}")):
        ...
        for j, subfolder in enumerate(subfolders):
            ...
            try:
                ...
                object_pixels = np.all(img_resized != [0, 0, 0], axis=-1)
        ...
        backbone = Featup()
        feature_point_cloud = fuse_feature_rgbd(backbone, imgs, dpts, msks, cameras, model_points)
        ...
        o3d.io.write_point_cloud(os.path.join(feature_object_path, "back_proj.pcd"), pcd)
    ...
    with h5py.File(output_path, 'w') as h5_file:
        for obj_name, obj_data in category_data.items():
            # Create group for this object
            obj_grp = h5_file.create_group(obj_name)
...
def main():
    ...
    data_root = '/home/q672126/objaverse/data'  # Root directory containing category folders
    output_dir = '/home/q672126/objaverse/h5py_files'  # Directory for output h5py files
    feature_model_dir = '/home/q672126/objaverse/feature_model'
    ...
    for category in categories:
        ...
        if os.path.isdir(category_path):
            output_file = os.path.join(output_dir, f"{category}.h5")
            feature_model_path = os.path.join(feature_model_dir, category)
            result_file = process_category(category_path, output_file, feature_model_path, id_info)
